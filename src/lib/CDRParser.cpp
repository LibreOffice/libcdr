...
void libcdr::CDRParser::readTxsm16(WPXInputStream *input)
{
  ...
  try
  {
    ...
    if (!frameFlag)
    {
      ...
      if (m_version < 1700)
        tlen *= 2;
      input->seek(tlen + 4, WPX_SEEK_CUR);
    }
    else
    {
...
void libcdr::CDRParser::_readX6StyleString(WPXInputStream *input, unsigned length, libcdr::CDRCharacterStyle &style)
{
